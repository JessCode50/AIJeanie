...
<template>
  ...
  <div v-else-if="view === 'AI'">
      ...
      <button @click="aiView()" :style="{marginTop: '40px'}">
        View a Session
      </button>
...
export default {
  ...
  mounted() {
    fetch('http://localhost:8082/contacts',{
        method: 'GET'
  ...
  methods: {
    ...
    editedContact(){
      fetch('http://localhost:8082/contacts/edit',{
        method: 'POST',
    ...
    newContact(){
      fetch('http://localhost:8082/contacts/new',{
        method: 'POST',
        body:JSON.stringify (this.selectedContact)
      })
      this.view = 'contacts'
    ...
    deleteContact(){
      fetch('http://localhost:8082/contacts/delete',{
        method: 'POST',
        body:JSON.stringify (this.selectedContact)
      })
